            ...
            <div class="col-12">
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            @if (isset($selectedAset) || isset($pemeliharaan))
                                <input type="text" class="form-control"
                                    value="{{ isset($selectedAset) ? $selectedAset->nama_aset . ' - ' . $selectedAset->merek . ' - ' . $selectedAset->kode_aset : (isset($pemeliharaan) ? $pemeliharaan->asset->nama_aset . ' - ' . $pemeliharaan->asset->merek . ' - ' . $pemeliharaan->asset->kode_aset : '') }}"
                                    disabled>
                                <input type="hidden" name="aset_pelihara"
                                    value="{{ isset($selectedAset) ? $selectedAset->id : (isset($pemeliharaan) ? $pemeliharaan->asset->id : '') }}">
                            @else
                                <select class="form-control select2" id="aset_pelihara" name="aset_pelihara">
                                    ...
                                    @foreach ($asets as $aset)
                                        <option value="{{ $aset->id }}"
                                            {{ (isset($selectedAset) && $selectedAset->id == $aset->id) || (isset($pemeliharaan) && $pemeliharaan->aset_pelihara == $aset->id) ? 'selected' : '' }}>
                        ...
                        <!-- Tambahkan ini untuk menampilkan ruangan asal aset -->
                        <div class="form-group">
                            <label for="ruangan_asal">Ruangan Asal</label>
                            <input type="text" class="form-control" id="ruangan_asal" name="ruangan_asal" value="{{ isset($selectedAset) ? $selectedAset->ruanganAsal->nama_ruangan : (isset($pemeliharaan) ? $pemeliharaan->asset->ruanganAsal->nama_ruangan : '') }}" disabled>
                            <input type="hidden" name="ruangan" value="{{ isset($selectedAset) ? $selectedAset->ruangan_asal : (isset($pemeliharaan) ? $pemeliharaan->asset->ruangan_asal : '') }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="jenis_pemeliharaan" name="jenis_pemeliharaan">
                                ...
                                @foreach ($jenpel as $j)
                                    <option value="{{ $j->id }}"
                                        {{ isset($pemeliharaan) && $pemeliharaan->jenis_pemeliharaan == $j->id ? 'selected' : '' }}>
